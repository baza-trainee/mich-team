...
const UkrPoshta = ({ status, selectRadioInput, list, handleDelivery }) => {
  ...
  useEffect(() => {
    if (status) handleDelivery({ citiesName, branch });
  ...
  return (
    <Wrap status={status}>
      <RadioInput
        type="radio"
        name="delivery"
        value=" Укрпошта (на відділення )"
        onChange={selectRadioInput}
        label=" Укрпошта (на відділення )"
        checked={status}
      />
      <ComponentAnimate
        tag={motion.div}
